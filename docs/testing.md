...
- catalog tests for deterministic secondary-index metadata lifecycle (create/list/duplicate checks), snapshot round-trip persistence, and legacy snapshot-version compatibility.
- rule planner tests now also validate deterministic INSERT secondary-index maintenance intent derived from table metadata.
